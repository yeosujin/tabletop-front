...
const SignUpPage = () => {
    ...
    const handleBlur = (e) => {
        ...
        if (name === 'password' || name === 'confirmPassword') {
            ...
            name === 'loginId' &&
            /[^a-zA-Z0-9]/.test(formValues.loginId)
        ) {
            setError('아이디는 영어와 숫자만 사용할 수 있습니다.')
        } else {
            setError('')
    ...
    const handleEmailValidation = async () => {
        try {
            ...
            if (is_valid_format.value && is_smtp_valid.value) {
                ...
                alert(
                    '입력하신 이메일 주소가 유효하지 않습니다. 올바른 이메일을 입력해주세요.'
                )
                setEmailValid(false)
            } else if (is_valid_format.value && !is_smtp_valid.value) {
                alert(
                    '이메일 형식은 맞으나 인증된 이메일이 아닙니다. 이메일 인증이 필요합니다.'
                )
                sendVerificationCodeToEmail()
            }
        } catch (err) {
    ...
    const handlePhoneValidation = async () => {
        try {
            ...
            if (valid) {
                ...
                alert(
                    '입력하신 휴대폰 번호가 유효하지 않습니다. 올바른 휴대폰 번호를 입력해주세요.'
                )
                setPhoneValid(false)
            }
        } catch (err) {
    ...
    const validateForm = () => {
        const { loginId, email, password, confirmPassword, username, mobile } =
            formValues
